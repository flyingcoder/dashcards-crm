...
export default {
    ...
    methods: {
        loadProjectReports() {
            this.loading = true
            makeRequestTo.get_project_reports(this.id)
        ...
        reportUpdated({data, index}) {
            let search_index = this.reports.findIndex(i => i.id === data.id)
            if (~search_index) {
                this.reports.splice(search_index, 1, data)
                this.active_report = search_index
            }
            this.$event.$emit('btnloading_off', false)
        },
        ...
        deleteReport() {
            ...
            apiTo.deleteProjectReport(this.id, this.deleteReportId)
                .then(() => {
