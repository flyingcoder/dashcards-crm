...
export default {
    ...
    computed: {
        googleBtnLabel() {
            return this.isGoogleSignIn ? 'Disconnect from Google Drive' : 'Connect to Google Drive'
        }
    },
            ...
            if (!this.isGoogleSignIn) {
                this.$gapi.signIn()
                    .then(user => {
                        ...
                        this.$event.$emit('open_snackbar', 'Successfully connected! ', 'success')
                    })
                    .catch(err => {
            ...
            if (this.isGoogleSignIn) {
                this.$gapi.signOut()
                    .then(() => {
                        ...
                        this.$event.$emit('open_snackbar', 'Successfully disconnected! ', 'success')
                    })
                    .catch(err => {
                        this.$event.$emit('open_snackbar', 'Error! ' + err.error, 'error')
                    })
