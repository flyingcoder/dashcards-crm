<template>
    <div class="chat">
        <GroupChatDialog ref="creategroupchat" @newChatGroupCreated="new_groupchat_added"></GroupChatDialog>
        <ManageMembersDialog ref="manage_group_members_dialog" :conversation="activeChat"></ManageMembersDialog>
                        ...
                        <v-card outlined class="mx-auto">
                            <div class="friend__list">
                                ...
                                <v-list subheader dense v-if="all_groups.length > 0">
                                    <v-list-item v-for="conv of all_groups" :key="conv.id" @click="open_conversation(conv)">
                                        <v-list-item-content>
                                ...
                                <v-list subheader dense>
                                    <v-list-item v-for="user of sorted(all_users)" :key="user.id" @click="open_conversation(user)">
                                        <v-list-item-content>
                ...
                <v-flex md8 sm7 xs12 class="chat__message">
                    ...
                    <v-card outlined class="mx-auto message__content" v-else-if="activeChat">
                                        ...
                                        <div class="info__two" v-show="activeChat.type !== 'group'">
                                            <div class="info__cta">
                                                <div class="info__anchor">
                                                    ...
                                                    <a href="javascript::void(0)" @click="go_to_profile(activeChat)">View Profile</a>
                                                </div>
                                                <div class="info__anchor">
                                                    <v-icon>email</v-icon>
                                                ...
                                                <div class="info__anchor">
                                                    ...
                                                    <a href="javascript::void(0)" @click="open_conversation(activeChat)">Chat</a>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <v-flex xs12 sm12 class="message__body">
                            <v-skeleton-loader v-if="cardLoading" ref="skeleton" type="card-avatar" class="mx-auto"></v-skeleton-loader>
                            <div class="messages" v-else-if="all_messages.length > 0" id="messageContainer">
                                <div class="view__more" v-show="has_more_message">
                                    <v-btn class="overline" block text dense :disabled="view__more_loading" :loading="view__more_loading" @click="get_previous_message">VIEW MORE</v-btn>
                                </div>
                                <Message v-for="message in all_messages" :message="message" :key="message.id"></Message>
                            </div>
                            <div class="messages-empty" color="#f5f7fa" v-else>
                                <Empty icon="mdi-message-plus-outline" slug="empty-chat" headline="Start sending messages"></Empty>
                            </div>
                            <div class="write px-2" v-if="!cardLoading">
                                <ChatField class="mt-2" :mentionables="mentionables" @typing="handleTyping" @send-message="sendMessage"></ChatField>
                            </div>
                        </v-flex>
                    </v-card>
                </v-flex>
