            ...
            <v-card>
                ...
                <v-card-text>
                    <v-container>
                        <v-row>
                            <v-flex xs12>
                                <v-text-field flat outlined solo prepend-inner-icon="mdi-dialpad" clearable label="Group Name*" v-model="group_name" required>
                                </v-text-field>
                            </v-flex>
                        </v-row>
                        <v-row>
                            <v-flex sm12 md8>
                                <v-text-field flat outlined solo label="Search for people to add" required clearable prepend-inner-icon="search" v-model="search" @keydown="filter_users" @click:clear="filter_users"></v-text-field>
                                <v-list v-if="filtered_by_search.length > 0" id="filtered_by_search_wrapper">
                                    <v-list-item v-for="item in filtered_by_search" :key="item.title" @click="selected(item)">
                                        <v-list-item-avatar>
                                        ...
                                        <v-list-item-icon>
                                            <v-icon v-show="item.is_selected" color="success">mdi-checkbox-marked-circle</v-icon>
                                        </v-list-item-icon>
                                    </v-list-item>
                                </v-list>
                            </v-flex>
                            <v-flex sm12 md4>
                                <v-list id="all_selected_wrapper">
                                    ...
                                    <v-list-item v-if="all_selected.length > 0" v-for="item in all_selected" :key="item.title">
                                        <v-list-item-avatar>
                                            <v-img :src="item.image_url"></v-img>
                                        </v-list-item-avatar>
                                        <v-list-item-content>
