...
export default {
    ...
    data: () => ({
        paths: [
            ...
            { text: 'Subscribers', disabled: true, router_name: null }
        ],
        sortList: [
            { title: 'Sort by name' },
            { title: 'Sort by email' },
    ...
    methods: {
        ...
        load_users() {
            this.fill_table_via_url(`api/company/subscribers?per_page=12`)
        },
        load_more_users() {
            this.load_more_via_url(`api/company/subscribers?per_page=12`)
        },
        toggleAll() {
