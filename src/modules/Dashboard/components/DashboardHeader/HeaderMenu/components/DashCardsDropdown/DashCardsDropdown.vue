<template>
    <v-menu class="datacards__dropdown" transition="slide-y-transition" :nudge-bottom="5" :nudge-left="180" offset-y :close-on-content-click="false" v-model="dropdown_visible">
        <template v-slot:activator="{ on }">
            <div class="d__dropdown" v-on="on">
                <header-icon :image-src="require('@/assets/icons/header/add__default.png')" :hovered-image-src="require('@/assets/icons/header/add.png')" />
            </div>
        </template>
        <v-list class="add__cards_list">
            ...
            <template>
                ...
                <div class="dash__cards">
                    <DashCard v-for="item of dash_items" :title="item.name" v-if="is_dashcard_enabled(item.slug)" :img-src="item.img_path" :selected="selected_cards" :id="item.id" :key="item.id" @click="check_selected(item.id)" />
                </div>
                <div class="card__btn">
