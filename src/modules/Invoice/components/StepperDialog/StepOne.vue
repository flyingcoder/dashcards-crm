<template>
    <v-row align="center" no-gutters class="step-one">
        ...
        <v-col cols="12" md="9" sm="12" class="value" v-if="!(dialog.type === 'create')">
            <v-text-field :disabled="dialog.type === 'edit'" outlined dense class="textfield" color="#657186" solo
                          hide-details flat placeholder="#" :value="`#INV-` + invoice_id"
            />
        </v-col>
        ...
        <v-col cols="12" md="9" sm="8" xs="12" class="value">
            <v-text-field class="titlefield" color="#657186" solo hide-details flat dense outlined
                          placeholder="Add Invoice Title *" v-model.trim="title"
            />
        </v-col>

        <v-col cols="12" md="3" sm="4" xs="12" class="label">Select Type <span class="required">*</span></v-col>
        <v-col cols="12" md="9" sm="8" xs="12" class="value">
            <v-select :disabled="projectLock" solo outlined full-width dense hide-details color="#657186" flat
                      :items="['hourly', 'monthly']" v-model="type" placeholder="Select Invoice Type"
            />
        </v-col>
        <v-col cols="12" md="3" sm="4" xs="12" class="label" v-if="type === 'hourly'">
            Project/Campaign <span
                class="required"
        >*</span>
        </v-col>
        <v-col cols="12" md="9" sm="8" xs="12" class="value" v-if="type === 'hourly'">
            <v-select :disabled="projectLock" :clearable="!projectLock" solo hide-details color="#657186" flat
                      full-width outlined dense :items="projects" :value="selected_project" @change="setBillingTargets"
                      item-text="title" item-value="id" placeholder="Select Project/Campaign"
            />
        </v-col>
        <v-col cols="12" md="3" sm="4" xs="12" class="label">Invoice Date <span class="required">*</span></v-col>
        <v-col cols="12" md="9" sm="8" xs="12" class="value">
            <date-picker :hasButtons="false" prepend-inner-icon="date_range" class="textfield" dense outlined
                         hide-details color="#657186" flat placeholder="Select Date" :value="date" :max="due_date"
        ...
        <v-col cols="12" md="9" sm="8" xs="12" class="value">
            <date-picker :hasButtons="false" prepend-inner-icon="date_range" class="textfield" dense outlined
                         hide-details color="#657186" flat placeholder="Select Date" :value="due_date" :min="date"
        ...
        <v-col cols="12" md="9" sm="8" xs="12">
            <v-autocomplete class="value" color="#657186" hide-details v-model="billed_from" :items="members" chips
                            item-text="fullname" item-value="id" solo full-width outlined flat dense
                            :value="billed_from" append-outer-icon="mdi-account-plus"
                            @click:append-outer="show_add_member_dialog('billed_from')"
            >
        ...
        <v-col cols="12" md="9" sm="8" xs="12">
            <v-autocomplete class="value" color="#657186" hide-details v-model="billed_to" :items="members" chips
                            item-text="fullname" item-value="id" solo full-width outlined flat dense :value="billed_to"
                            append-outer-icon="mdi-account-plus"
                            @click:append-outer="show_add_member_dialog('billed_to')"
            >
                <template v-slot:selection="data">
                    <v-chip v-bind="data.attrs" tile outlined>
                        <v-avatar left>
        ...
        <teams-dialog ref="add_dialog" title="Add New Member" :dialog.sync="add_dialog" @save="add_member"
