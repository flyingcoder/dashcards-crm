<template>
    <v-row no-gutters>
        ...
        <v-col md="9" sm="8" class="value mb-2">
            <v-textarea label="Notes - any revelant information not already covered" :value="notes" :disabled="dialog.type === 'view'" class="textarea__field" flat solo dense outlined :rows="2" hide-details color="#657186" @input="update_textarea($event, 'notes')" />
        </v-col>
        <v-divider></v-divider>
        <v-col md="3" sm="4" class="label mb-2">Terms</v-col>
        <v-col md="9" sm="8" class="value mb-2">
            <v-textarea label="Terms and conditions - late fees, payment methods, delivery schedule" :value="terms" :disabled="dialog.type === 'view'" class="textarea__field" flat solo dense outlined :rows="2" hide-details color="#657186" @input="update_textarea($event, 'terms')" />
        </v-col>
    </v-row>
</template>
<script>
