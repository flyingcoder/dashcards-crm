<template>
    <div class="invoice-card">
        <div class="invoice__content">
            <dash-card title="Invoice" :dashboard="dashboard" @close="$emit('close')" :view-more-link="viewMoreLink" :viewMoreBtn="enableViewMore" @view-more="">
                <div slot="actions"></div>
                <div class="content-wrapper" slot="content">
                    <v-data-table :headers="headers" :items="items" hide-default-footer shaped>
                        <template v-slot:top>
                            <v-row no-gutters>
                                <v-spacer></v-spacer>
                                <v-col md="6" sm="12">
                                    <v-select class="dialog__selectfield" solo hide-details color="#657186" :items="['Remind', 'Cancel', 'Archive']" label="Batch Actions"></v-select>
                                </v-col>
                            </v-row>
                        </template>
                            ...
                            <tr>
                                ...
                                <td>
                                    <v-menu bottom left>
                                        ...
                                        <v-list>
                                            <v-list-item v-for="(item, i) in [ 'Copy', 'Print', 'PDF',  'Share link' ]" :key="i" @click="">
