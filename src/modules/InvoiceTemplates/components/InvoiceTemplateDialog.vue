                    ...
                    <v-col grow>
                        ...
                        <v-row no-gutters class="mt-2">
                            <v-col grow>
                                ...
                                <input type="file" @change="getFile" ref="inputfile" accept=".txt,.html,.htm" style="display:none;">
                                <v-btn class="mr-1" @click="$refs.inputfile.click()">Upload File</v-btn>
                            </v-col>
                            <v-col md="5">
                                <v-select :value="null" clearable outlined @change="setStartingtemplate" :items="defaultTemplates" return-object item-text="name" label="Customize from default" flat dense :height="25" solo></v-select>
                            </v-col>
                        </v-row>
                        <v-textarea :rows="67" v-model="html" clearable :value="html" placeholder="Template HTML" auto-grow solo hide-details flat></v-textarea>
                    </v-col>
                    <v-col md="4">
                        <v-list three-line dense v-if="invoicefields">
                            ...
                            <v-list-item style="min-height:25px;" v-for="(info, field) in invoicefields" :key="field">
                                <v-list-item-content>
                                    ...
                                    <v-list-item-subtitle v-html="info.description"></v-list-item-subtitle>
                                </v-list-item-content>
                                <!-- <v-list-item-icon>
                                    <v-tooltip left>
