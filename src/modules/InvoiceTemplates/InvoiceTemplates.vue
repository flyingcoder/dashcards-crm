    ...
    <div class="invoice-templates">
        <VueTable title="Invoice Templates" :hasSearch="false" :headers="headers" :loading="loading" :items="items" :noMoreData="noMoreData" @load-more="getMoreInvoiceTemplates" :showSelect="false">
            ...
            <template v-slot:row-slot="{ item }">
                ...
                <td v-if="item.company_id === 0" />
                <td v-else>{{ item.created_at | bzFromNow }}</td>
                <Actions :hasDelete="can_action(item) && can_delete(item)" :hasEdit="can_action(item)" :item="item" :permissions="$_permissions.get('templates')" @delete="open_delete_dialog(item)" @edit="open_edit_dialog(item)" @view="open_view_dialog(item)" />
            </template>
            <template v-slot:empty-slot>
                <v-btn dark color="#3b589e" @click="open_add_dialog">Add new invoice template</v-btn>
            </template>
        </VueTable>
        <InvoiceTemplateDialog ref="add_edit_dialog" :defaultTemplates="defaultTemplates" :invoicefields="invoicefields" @save="handleSave" @preview="open_view_dialog" />
        <InvoiceTemplatePreview ref="view_dialog" :item="activeItem" :invoicefields="invoicefields" />
        <InvoiceTemplateHelp ref="help_dialog" :invoicefields="invoicefields" />
