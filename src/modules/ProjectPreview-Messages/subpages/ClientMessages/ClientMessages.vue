<template>
    <div class="message__body team-messages">
        <v-system-bar lights-out :height="40" v-if="can_manage_members">
            <v-spacer />
            <v-btn text small class="primary-btn" @click="open_manage_member_dialog">
        ...
        <div class="messages" ref="messages-container">
            <v-row no-gutters class="pa-3" v-if="can_send_message">
                <v-spacer />
                <v-col>
                    <v-btn text v-if="noMoreData === false && !loading" class="view__more_btn overline"
                           @click="load_previous"
                    >
            ...
            <div class="no-messages" v-else-if="!can_send_message">
                <Empty icon="mdi-cellphone-off" headline="Client messages unavailable! " />
            </div>
            <div class="no-messages" v-else>
                <Empty icon="mdi-cellphone-message-off" headline="No messages yet" />
            </div>
        </div>
        ...
        <ManageClientChatMember ref="manage_group_members_dialog" :conversation="activeChat" />
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...
            can_manage_members() {
                ...
                let role = Object.values(this.loggedUser.user_roles)[0];
                return !!(~role.indexOf('manager'))
            },
            can_send_message() {
