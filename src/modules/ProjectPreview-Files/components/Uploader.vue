<template>
  <v-card flat outlined class="uploader">
    <v-card-text
      class="preview-wrapper text-center"
    ...
    <v-card-actions class="pa-2 actions">
      ...
      <v-btn
        ...
        text
        outlined
        :disabled="isMaxFile || btnloading"
        @click="$refs.hidden_file_input.click()"
      >
      ...
      <v-btn
        large
        outlined
        v-if="can_create_folder"
        @click="$emit('open-create-folder-dialog')"
      >
