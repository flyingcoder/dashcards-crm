<template>
    <div class="link-dialog">
        <CustomDialog ref="dialog" :mainBtnDisabled="!hasPreferredFolders" :open.sync="dialog"
                      title="Enter Google Drive Folder URL" button1-text="Cancel" @button1="close_dialog"
                      button2-text="Save" @button2="saveClicked"
        >
            <template #content>
                <v-row class="custom-dialog">
                    ...
                    <v-col md="12">
                        <v-row>
                            <v-col md="10">
                                <v-text-field :disabled="maxFolderExceed" label="Enter Google Drive Folder URL"
                                              clearable v-model.trim="url" type="url" filled hide-details
                                              color="#657186"
                                />
                            </v-col>
                            <v-col>
                                <v-tooltip left>
                                    <template v-slot:activator="{ on }">
                                        <v-btn :disabled="maxFolderExceed" @click="getFolderPreview" large icon outlined
                                               v-on="on"
                                        >
                            ...
                            <v-card-actions>
                                <v-list-item class="grow">
                                    <v-list-item-avatar tile color="grey darken-3">
                                        <v-img class="elevation-6" :src="folder_icon" />
                                    </v-list-item-avatar>
                                    <v-list-item-content>
                                    ...
                                    <v-row align="center" justify="end">
                                        <v-tooltip left>
                                            <template v-slot:activator="{ on }">
                                                <v-icon v-on="on" @click="delete_from_preffered(folder)" class="mr-1">
                                                    mdi-close
                                                </v-icon>
                                            </template>
                            ...
                            <v-card-actions>
                                <v-list-item class="grow">
                                    ...
                                    <v-row align="center" justify="end">
                                        <v-tooltip left>
                                            <template v-slot:activator="{ on }">
                                                <v-icon v-on="on" @click="delete_from_db(folder)" class="mr-1">
                                                    mdi-folder-remove
                                                </v-icon>
                                            </template>
