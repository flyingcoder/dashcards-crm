<template>
    ...
    <v-tooltip left>
        <template v-slot:activator="{ on }">
            <v-icon v-on="on" @click="delete_from_db(folder)" class="mr-1">mdi-folder-minus</v-icon>
        </template>
    ...
    methods: {
        ...
        getFolderPreview() {
            ...
            this.$gapi
                ...
                .catch(error => {
                    this.$event.$emit('open_snackbar', error.result.error.message, 'error')
                })
                .then(response => {
                    if (response.result.mimeType && response.result.mimeType.includes('.folder')) {
        ...
        delete_from_db(item) {
            request.delete(`api/projects/${this.projectId}/folder-id/google-drive/${item.id}`)
                .then(response => {
