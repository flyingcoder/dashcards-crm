        ...
        <v-card outlined class="gdrive-content">
            <v-card-title class="py-1">
                <v-col md="12" v-if="isGoogleAuthorized">
                    ...
                    <v-alert type="info" outlined v-else-if="activeFolderId === 'root'">
                ...
                <v-btn large text outlined @click="openAddFolderDialog" v-if="can_manage_folders && isGoogleAuthorized">
                    <v-icon left >mdi-folder-google-drive</v-icon> Manage Google Drive Folders
                </v-btn>
            ...
            <v-card-text>
                ...
                <Empty v-if="!isGoogleAuthorized && !loading" headline="No Google Drive connected!">
                    <template v-slot:extra>
                        <v-btn large dark color="#3b589e" :to="{ name: 'cloud' }">
                            <v-icon left>mdi-google-drive</v-icon> Connect to Google
                            Drive
                        </v-btn>
                    </template>
                </Empty>
                <v-row class="my-0" v-else-if="files.length > 0 && !loading">
                    <v-col md="2" sm="3" v-for="file in files" :key="file.id">
                        <v-card flat outlined>
                            <v-img contain :src="getImage(file)" class="white--text align-end" gradient="to bottom, rgba(0,0,0,.1), rgba(0,0,0,.4)" height="150">
                            ...
                            <v-card-actions>
                                ...
                                <v-menu bottom v-if="file.exportLinks && file.capabilities.canDownload" origin="center center" transition="slide-x-transition">
                                    ...
                                    <v-list class="pa-0 ma-0">
                                        <v-list-item v-for="(item, i) in links(file.exportLinks)" :key="i" class="ma-0 pa-0">
                                ...
                                <v-tooltip bottom v-else-if="file.webContentLink && file.capabilities.canDownload">
                                    <template v-slot:activator="{ on }">
                ...
                <Empty v-else-if="!loading && files.length === 0" headline="No files found!"></Empty>
            </v-card-text>
        </v-card>
    </div>
</template>
