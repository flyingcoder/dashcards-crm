        ...
        <VueTable :items="items" :headers="headers" :showRowActions="true" icon="widgets" title="Permissions"
            ...
            <template v-slot:header-toolbar>
                <v-select solo flat outlined dense hide-details class="col-md-3 permissions-selection"
                          label="Select Group" :items="groups" v-model="selected_group"></v-select>
            </template>
                ...
                <td>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-icon :color="getCapability(item, 1)" v-bind="attrs" v-on="on" class="mx-2">mdi-account-check-outline</v-icon>
                        </template>
                        <span>Can view</span>
                    </v-tooltip>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-icon :color="getCapability(item, 2)" v-bind="attrs" v-on="on" class="mx-2">mdi-account-multiple-plus-outline</v-icon>
                        </template>
                        <span>Can create</span>
                    </v-tooltip>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-icon :color="getCapability(item, 3)" v-bind="attrs" v-on="on" class="mx-2">mdi-account-edit-outline</v-icon>
                        </template>
                        <span>Can update</span>
                    </v-tooltip>
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-icon :color="getCapability(item, 4)" v-bind="attrs" v-on="on" class="mx-2">mdi-account-multiple-remove-outline</v-icon>
                        </template>
                        <span>Can delete</span>
                    </v-tooltip>
                </td>
                <Actions :item="item" :hasDelete="false" :hasView="false"
                         :permissions="$_permissions.get('permissions')" @edit="open_edit_dialog(item)"></Actions>
            </template>
        </VueTable>
    </div>
</template>
