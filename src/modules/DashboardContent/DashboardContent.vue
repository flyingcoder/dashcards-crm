            ...
            <div v-if="is_screen_medium_and_up" class="desktop-cards">
                <draggable class="row d__cards" v-model="cards" scroll-sensitivity="300" force-fallback="true">
                    <template v-for="card in card_components">
                        <v-col cols="12" :key="card.slug" md="6" v-if="should_show(card.slug) && card.can_view() && is_dashcard_enabled(card.slug)">
                            <component :is="card.component" :key="card.component" v-bind="args" v-if="card.hasOwnProperty('component')" @close="close(card.id)" />
                            <template v-else>
                                <v-flex xs12 :key="card.id">
                                    <div>
                                        <dash-card @close="close(card.id)" :title="card.name" :dashboard="args.dashboard" />
                                    </div>
                                </v-flex>
                            </template>
                        </v-col>
