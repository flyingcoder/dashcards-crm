<template>
    <div class="empty-cards">
        <CustomDialog title="Select Dash Cards" ref="dialog" :open.sync="dialog" @button1="dialog = false">
            <template slot="content">
                <div class="dash__cards">
                    <template v-for="item of dash_items" v-if="is_dashcard_enabled(item.slug)">
                        <DashCard :title="item.name" :img-src="item.img_path" :selected="selected" :id="item.id" :key="item.id" @click="check_selected(item.id)" />
                    </template>
                </div>
            </template>
