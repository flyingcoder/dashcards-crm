<template>
    <div class="add-picture">
        <CustomDialog ref="picture_dialog" title="Upload New Profile Picture" button2-text="Save" @button1="cancel" :open.sync="dialog">
            <template #content>
                <div class="content">
                    <Loader :loading="loading" />
                    <CropImage v-if="file_uploaded" ref="croppie" :image="image64" :options="croppie.options" :result="croppie.result" />
                    <CustomDropzone v-else ref="dropzone" :duplicateCheck="true" acceptedFiles="image/*" :options="dropzoneOptions" :useCustomSlot="true" dictFileTooBig="File too big" dictInvalidFileType="Invalid file type" @file-added="file_added" />
                </div>
            </template>
            <template #button2>
