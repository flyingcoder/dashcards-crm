<template>
    <div class="projects">
        <table-header :paths="paths" :noListButton="false" :noGridButton="false" @click="add_dialog = true" @click-list-view="view = `list`" @click-grid-view="view = `grid`" />
                ...
                <v-card class="mx-auto">
                    <v-card-title>
                        <Avatar :user="item.project_client.user" iconOnly>
                          <template>
                            <span class="subtitle-2">{{item.project_client.user.fullname | truncate(20) }}</span>
                          </template>
                        </Avatar>
                    </v-card-title>
                    <v-divider></v-divider>
