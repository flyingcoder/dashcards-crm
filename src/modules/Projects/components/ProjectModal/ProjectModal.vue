  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" scrollable max-width="700px" persistent>
      <v-card>
        ...
        <v-card-text style="height: 500px;">
          <v-row no-gutter>
            ...
            <v-col md="4" sm="6" >
              ...
              <div class="choosen" v-if="date_pickers.start_date !== '' || date_pickers.end_date !== ''">
            ...
            <v-col md="8" sm="12">
              <v-menu bottom close-on-content-click transition="slide-y-transition" bottom max-height="300">
                <template v-slot:activator="{ on }">
                  <v-btn block outlined tile text  v-on="on" >
                    <v-icon left >people_outline</v-icon><v-icon left>add</v-icon> Assign Members
                  </v-btn>
                </template>
                <v-list dense >
              ...
              <div class="choosen" v-if="members.selected.length > 0">
                <v-chip outlined class="mt-1 tile"
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn outlined text @click="cancel"><v-icon left>close</v-icon> Close</v-btn>
          <v-btn outlined :loading="btnloading" :disabled="disabled" @click="save"><v-icon left>save</v-icon> Save</v-btn>
          <v-spacer></v-spacer>
        </v-card-actions>
      </v-card>
    </v-dialog>
