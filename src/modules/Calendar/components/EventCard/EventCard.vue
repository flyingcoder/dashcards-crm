<template>
  <div class="event_card">
    <div class="card_header">
      <h3 class="card_title">
        <v-icon small :color="event.event_type.properties.color"
      ...
      <v-menu bottom left v-if="dense">
        ...
        <v-list>
          <v-list-item @click="$emit('edit-event')">
            <v-list-item-title
              ><v-icon left>edit</v-icon> Edit</v-list-item-title
            >
          </v-list-item>
          <v-list-item @click="$emit('delete-event')">
            <v-list-item-title
              ><v-icon left>delete</v-icon> Delete</v-list-item-title
            >
          </v-list-item>
          <v-list-item @click="$emit('view-event')">
            <v-list-item-title
              ><v-icon left>pageview</v-icon> View</v-list-item-title
            >
          </v-list-item>
        </v-list>
      </v-menu>
