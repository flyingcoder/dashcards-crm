...
export default {
  ...
  data: () => ({
    ...
    event_type : null,
    dialogKey : 0,
    createDateFormat : { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit'}
	}),
  computed: {
    disabled() {
      if (!this.all_day && (!this.description || !this.event_type || !this.title || !this.start_date || !this.end_date )){
        return true
      }
      if (this.all_day && (!this.description || !this.event_type || !this.title || !this.start_date)) {
      	return true
      }
      return false
    }
      ...
      setTimeout(() => {
		...
		save() {
			...
			var payload = {
				...
				end_date : moment(!this.all_day ? this.end_date : this.start_date).format('YYYY-MM-DD HH:mm:ss'),
