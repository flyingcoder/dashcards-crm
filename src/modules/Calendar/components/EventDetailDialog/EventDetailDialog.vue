                ...
                <v-card-text style="height: 500px;">
                    <v-row no-gutters>
                        ...
                        <v-btn icon outlined v-if="can_edit_event" class="mr-1">
                            <v-icon @click="$emit('edit-event')">edit</v-icon>
                        </v-btn>
                        <v-btn icon outlined v-if="can_delete_event" class="mr-1">
                            <v-icon @click="$emit('delete-event')">delete</v-icon>
                        </v-btn>
                    ...
                    <v-row no-gutters v-if="event">
                        ...
                        <v-col md="4" class="pa-4">
                            <Avatars :items="event.participants" :hasAddMember="can_add_participants" :count="7" :deep="true" @add-member="$emit('add-participant')"></Avatars>
                        </v-col>
                        <v-col md="12" class="pa-4">
                        ...
                        <v-col>
                            <v-divider></v-divider>
                            <Comment :items="all_comments" :addApi="addCommentApi" :deleteApi="deleteCommentApi"></Comment>
                        </v-col>
                    </v-row>
                </v-card-text>
            </v-card>
