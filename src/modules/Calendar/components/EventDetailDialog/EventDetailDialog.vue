    ...
    <v-dialog v-model="dialog" scrollable max-width="600px" persistent>
      <v-card class="custom__dialog">
        ...
        <v-card-text style="height: 500px;">
          <v-row no-gutters>
              <label>{{event_date}}</label> <v-divider class="mx-2" vertical ></v-divider> <label>{{event_time}}</label>
              <v-spacer></v-spacer>
              <v-btn icon outlined class="mr-1"><v-icon @click="$emit('edit-event')">edit</v-icon></v-btn>
              <v-btn icon outlined class="mr-1"><v-icon @click="$emit('delete-event')">delete</v-icon></v-btn>
          ...
          <v-row no-gutters>
            <v-col md="12" class="pa-4">
              <v-avatar size="20" :color="event.event_type.properties.color"></v-avatar> 
              <span class="font-weight-medium title"> {{event.event_type.name}}</span>
            </v-col>
...
<style scoped>
>>> .rich-editor .ql-editor { min-height:100px } 
</style>