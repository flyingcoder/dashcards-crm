...
export default {
  ...
  computed: {
    ...
    user_participants() {
      return this.event.participants.map( ii => { return ii.user })
    }
  },
  methods: {
  	openDialog() {
      ...
      var payload = { participants : this.to_be_added.map(o => { return o.id }) }
      makeRequestTo.addParticipants(this.event.id, payload)
      .then(({ data }) => {
    ...
    remove_from_selected_members(item) {
      let index = this.to_be_added.findIndex(
