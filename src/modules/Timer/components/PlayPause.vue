	...
	export default {
		...
		computed :{
			...
			is_viewer_a_participant(){
				let indx = this.participants.findIndex(i => i.id === this.viewer.id)
	    ...
	    timerClick() {
	      ...
	      if (this.timer_started) this.pause_timer()
	        ...
	        this.item.timer.timer_disabled = true
	        request.post(api, { type: 'task', id: this.item.id })
	        .then(() => {
	          this.item.timer.timer_disabled = false
	    ...
	    start_timer() {
	      ...
	      this.item.timer.timer_interval = setInterval(() => {
	        ...
	        this.item.timer.timer_stats.format = this.set_time_running(hours, mins, secs)
	      }, 1000)
	    },
