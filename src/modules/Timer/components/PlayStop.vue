<template >
	<v-row class="timer my-0 py-0" >
		...
		<v-col class="time-control">
			<v-tooltip left v-if="is_ongoing_previous_date || is_lapsed">
	...
	export default {
		...
		computed :{
			...
			is_ongoing_previous_date() {
				...
				return this.item.timer.latest_timer.action === 'start' 
					&& this.item.timer.total_seconds === 0 
					&& this.item.timer.timer_status === ''
			},
			last_date_action(){
				return moment(this.item.timer.latest_timer.created_at).format('MMMM D YYYY')
			}
		},
		created(){
			if (this.is_open()) {
		        	...
		        	if (type === `start`) {
		        		this.setGlobalTimerRunning()
								...
								if (!this.item.timer.timer_created) {
									this.item.timer.timer_created = moment().format('YYYY-MM-DD HH:mm:ss')
								}
		        	} else {
		        		clearInterval(this.interval)
		        		this.item.timer.timer_status = 'close'
		        		this.timer_started = false
		        		this.item.timer.timer_stopped = moment().format('YYYY-MM-DD HH:mm:ss')
		        	}
