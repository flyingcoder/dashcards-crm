  ...
  <div class="alarm">
    ...
    >
      ...
      <template v-slot:row-slot="{ item }">
        ...
        <td><v-icon :color="item.event_type.properties.color" left>mdi-circle</v-icon> {{ item.event_type.name}}</td>
        <td>{{ item.start | format('MMM DD YYYY') }}</td>
        <td>{{ item.start | format('HH:mm:ss')}}</td>
        <td>
           <v-tooltip left v-if="is_event_owner(item)">
            <template v-slot:activator="{ on }">
              <v-icon small class="mx-1" v-on="on" @click="handleAction">delete</v-icon>
            </template>
            <span>Delete Event</span>
          </v-tooltip>
           <v-tooltip left v-if="is_event_owner(item)">
            <template v-slot:activator="{ on }" >
              <v-icon small class="mx-1" v-on="on" @click="handleAction">edit</v-icon>
            </template>
            <span>Edit Event</span>
          </v-tooltip>
            <v-tooltip left v-if="is_event_owner(item)">
            <template v-slot:activator="{ on }" >
              <v-icon small class="mx-1" v-on="on" @click="handleAction">pageview</v-icon>
            </template>
            <span>View Event</span>
          </v-tooltip>
          <v-tooltip left v-if="is_event_owner(item)">
            <template v-slot:activator="{ on }" >
              <v-icon small class="mx-1" v-on="on" @click="handleAction">mdi-email-send-outline</v-icon>
            </template>
            <span>Add Participant</span>
          </v-tooltip>
          <v-tooltip left v-else>
            <template v-slot:activator="{ on }" >
              <v-icon small class="mx-1" v-on="on" @click="handleAction" >mdi-email-receive-outline</v-icon>
            </template>
