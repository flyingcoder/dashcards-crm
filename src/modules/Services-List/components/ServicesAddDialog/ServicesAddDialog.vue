        ...
        <v-dialog v-model="open" persistent scrollable max-width="600px">
            <v-card class="addService__dialog">
                ...
                <v-card-text scrollable class="dialog__body">
                    <v-layout wrap>
                        <v-layout align-center justify-space-between class="service__field">
                            <v-flex xs10>
                                <v-text-field label="Service Name" class="service__input" v-model.trim="name" @keyup.enter="add_service" filled hide-details="auto" color="#657186"></v-text-field>
                            </v-flex>
                        ...
                        <v-flex xs12>
                            <v-layout align-center v-for="(service, index) of services" :key="index" class="service__field">
                                <v-flex xs10>
                                    <v-text-field label="Service Name" class="service__input" v-model.trim="service.name" filled hide-details color="#657186"></v-text-field>
                                </v-flex>
                                <v-flex xs2 class="text-xs-right">
                                    <v-btn class="deleteService__btn" @click="delete_service(index)">
                ...
                <v-card-actions class="service__actions">
                    ...
                    <v-btn class="service__actions_btn" :disabled="is_disabled" :loading="btnloading" @click="save">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-layout>
