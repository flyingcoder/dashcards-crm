    ...
    <div class="users-table">
        <div class="users-table-wrapper">
            <v-card flat class="users-table-inner">
                ...
                <v-progress-linear height="5" indeterminate v-if="loading" />
                <v-row>
                    <v-col md="3">
                            ...
                            <v-card-text>
                                <v-row no-gutters class="mx-auto">
                                    ...
                                    <v-avatar size="100" class="mb-3">
                                        <v-img :src="activeUser.image_url" />
                                    </v-avatar>
                                    <v-spacer />
                                    <v-col cols="12" />
                                    ...
                                    <v-col sm="8" cols="12" class="info-item">
                                        {{ activeUser.telephone ?
                                        activeUser.telephone.formatInternational : 'none' }}
                                    </v-col>
                                    <v-col sm="4" cols="12" class="info-label">Is company owner :</v-col>
                                    <v-col sm="8" cols="12" class="info-item">
                                        {{ activeUser.is_company_owner ? 'Yes' :
                                        'No' }}
                                    </v-col>
                                    ...
                                    >
                                        ...
                                        <v-btn class="disable-acc" right v-if="can_update_user_roles"
                                               :loading="restoring" text @click="controlAccount()"
                                        >
                                            <v-icon left small>
                                                {{ activeUser.deleted_at ? 'mdi-account-convert' :
                                                'mdi-account-lock' }}
                                            </v-icon>
                                    ...
                                    <v-col sm="8" cols="12">
                                        ...
                                        <v-autocomplete dense :disabled="!can_update_user_roles" append-icon="add" flat
                                                        item-text="name" v-model="tobeAddedRoles" :items="selectRoles"
                                                        outlined class="my-2" return-object chips small-chips multiple
                                                        solo
                                        >
                                            <template v-slot:append-outer>
                                                <v-tooltip left>
                                                    <template v-slot:activator="{ on } ">
                                                        <v-btn class="save-btn" :loading="btnloading" v-on="on" icon
                                                               @click="updateRoles"
                                                        >
                                                            <v-icon>save</v-icon>
                                                        </v-btn>
                                        ...
                                        <v-simple-table fixed-header>
                                            <template v-slot:default>
                                                ...
                                                <tr v-for="(perm,index) in permissions" :key="index">
                                                    ...
                                                    <td>
                                                        <v-icon left color="success" v-if="perm.create">
                                                            mdi-check-circle
                                                        </v-icon>
                                                        <v-icon left color="red" v-else>mdi-close-circle</v-icon>
                                                    </td>
                                                    <td>
                                                        <v-icon left color="success" v-if="perm.delete">
                                                            mdi-check-circle
                                                        </v-icon>
                                                        <v-icon left color="red" v-else>mdi-close-circle</v-icon>
                                                    </td>
                                                    <td>
                                                        <v-icon left color="success" v-if="perm.update">
                                                            mdi-check-circle
                                                        </v-icon>
                                                        <v-icon left color="red" v-else>mdi-close-circle</v-icon>
                                                    </td>
                                                    <td>
                                                        <v-icon left color="success" v-if="perm.view">
                                                            mdi-check-circle
                                                        </v-icon>
                                                        <v-icon left color="red" v-else>mdi-close-circle</v-icon>
                                                    </td>
