<template>
    <div class="clients">
        <clients-dialog :dialog.sync="add_dialog" ref="add_dialog" dialogTitle="Add New Client"
                        @save="add_item('add_new_client', $event)"
        />
        <clients-dialog :dialog.sync="edit_dialog" ref="edit_dialog" dialogTitle="Edit Client"
                        :is-edit-dialog="edit_dialog" :fields-to-edit="edit_item"
                        @save="update_item('update_client', $event)"
        />
        <delete-dialog :open-dialog.sync="delete_dialog" title="Delete Client"
                       text-content="Are you sure you want to delete this client?"
                       @delete="delete_item('delete_client')"
        />
        <delete-dialog :open-dialog.sync="bulk_delete_dialog" title="Delete Clients"
        ...
        <VueTable v-if="view === `list`" :items="items" :headers="headers" :showRowActions="true"
                  ...
                  :loading="loading" emptyText="No client yet" title="Clients" :key="componentKey"
                  :noMoreData="noMoreData" :showSelect="true"
                  @load-more="load_more" @delete-selected="open_bulk_delete_dialog($event)"
        >
            ...
            <template v-slot:row-slot="{ item }">
                ...
                <Actions :item="item" :permissions="$_permissions.get('clients')" @delete="open_delete_dialog(item)"
                         @edit="open_edit_dialog(item)" @view="navigate_to_view_profile(item.id)"
                />
            </template>
        ...
        <!-- Grid View  -->
        <VueGrid v-else icon="mdi-account-tie-outline" emptyText="No client yet" title="Clients" :showTaskCount="false"
                 :items="items" @load-more="load_more" :key="componentKey"
                 :noMoreData="noMoreData" @delete="open_delete_dialog" @edit="open_edit_dialog"
        >
