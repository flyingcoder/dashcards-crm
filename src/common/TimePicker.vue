<template>
    <div class="text-center">
        <v-menu :close-on-content-click="false" :nudge-width="200" offset-y :max-width="300" v-model="menu"
        ...
        >
            ...
            <v-card class="pa-0">
                <v-time-picker v-model="picker" :landscape="false" :use-seconds="hasSecond" ampm-in-title
                               :format="format" ref="vtimepicker"
                >
                    <v-row no-gutters>
                        <v-btn block class="mb-1" v-if="hasAlarm" :color="alarm ? `primary` : `gray`"
                               @click="toggleAlarm"
                        >
