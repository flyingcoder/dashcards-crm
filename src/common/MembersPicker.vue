          ...
          <v-list-item class="px-1">
            ...
            <template v-slot:selection="{ item }">
	            <v-chip
	              :key="item.id"
	              close
	              @click:close="deselectUser(item)"
	            >
	              <v-avatar class="accent white--text" left ><v-img :src="item.image_url"></v-img></v-avatar>
	        ...
	        <v-list-item :style="`max-height: ${maxHeight}px;`" class="nexted px-0" v-if="members.length > 0">
	        	...
	        	<v-list dense >
		            ...
		            <v-list-item-content>
		              <v-list-item-title>{{user.fullname | str_limit(20) }}</v-list-item-title>
		              <v-list-item-subtitle>{{user.job_title | str_limit(20) }}</v-list-item-subtitle>
		            </v-list-item-content>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMembersExcept(){
        var payload = { except : this.except.map(o => { return o.id }) }
        console.log(payload)
        makeRequestTo.getAllMembersExcept(payload)
        .then(({ data }) => {
          this.original = _cloneDeep(data)
          this.members = data
        })
    	...
    	getMemberIndex(user) {
    		let index = this.members.findIndex( member => user.id === member.id)
        return index 
    	},
    	getSelectedIndex(user) {
    		let index = this.selected.findIndex( member => user.id === member.id)
        return index 
    	},
    	setAsSelected(user) {
