<template>
    <div class="files-preview">
        <!-- for videos -->
        <template v-if="display_videos.length > 0">
            <v-tooltip left v-for="media in display_videos" :key="media.id">
                <template v-slot:activator="{ on }">
                    <v-avatar tile size="50" v-on="on" class="mr-1 clickable" @click="openViewer(media, 'video')">
                        <Media :media="media" size="md" />
                    </v-avatar>
                </template>
        ...
        <!-- for images -->
        <template v-if="display_medias.length > 0">
            <v-tooltip left v-for="media in display_medias" :key="media.id">
                <template v-slot:activator="{ on }">
                    <v-avatar v-on="on" tile class="mr-1 clickable" @click="openViewer(media, 'image')">
        ...
        <!-- for docs -->
        <template v-if="display_docs.length > 0">
            <v-tooltip left v-for="media in display_docs" :key="media.id">
                <template v-slot:activator="{ on }">
                    <v-avatar tile v-on="on" class="mr-1 clickable" @click="openViewer(media, 'doc')">
                        <Media :media="media" size="md" />
                    </v-avatar>
                </template>
                <span>{{ media.name }}</span>
        ...
        <!-- for others -->
        <template v-if="display_others.length > 0">
            <v-tooltip left v-for="media in display_others" :key="media.id">
                <template v-slot:activator="{ on }">
                    <v-avatar v-on="on" tile class="mr-1 clickable" @click="openViewer(media, 'other')">
                        <Media :media="media" size="md" />
                    </v-avatar>
                </template>
                <span>{{ media.name }}</span>
        ...
        <!-- for links -->
        <template v-if="display_links.length > 0">
            <v-tooltip left v-for="media in display_links" :key="media.id">
                <template v-slot:activator="{ on }">
                    <v-avatar v-on="on" tile class="mr-1 clickable" @click="openViewer(media, 'link')">
                        <v-img :src="getMediaSrc(media)">
                            <template v-slot:placeholder>
                                <Media :media="media" size="md" />
                            </template>
        ...
        <VideoViewer ref="video_viewer_dialog" :media="selected_media" />
        <ImageViewer ref="image_viewer_dialog" :media="selected_media" />
...
<script>
    ...
    export default {
        ...
        computed: {
            display_medias() {
                const images = this.item.attachments.filter(i => {
                    return i && i.category === 'images'
                });
            ...
            display_videos() {
                const videos = this.item.attachments.filter(i => {
                    return i && i.category === 'videos'
                });
            ...
            display_others() {
                const others = this.item.attachments.filter(i => {
                    return i && i.category === 'others'
                });
            ...
            display_docs() {
                const docxs = this.item.attachments.filter(i => {
                    return i && i.category === 'documents'
                });
            ...
            display_links() {
                const links = this.item.attachments.filter(i => {
                    return i && i.category === 'links'
                });
        ...
        methods: {
            getMediaSrc(media) {
                ...
                if (media.category === 'documents') {
                    ...
                    return urls[media.custom_properties.ext] || urls.docx
                }
