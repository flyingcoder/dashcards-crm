<template>
	<v-card flat>
	    ...
	    <template v-slot:item="{ item }">
			<slot name="row-slot" v-bind:item="item"></slot>
		</template>
	    ...
	    <template v-slot:footer v-if="hasFooter">
	    	...
	    	<v-row class="py-3 px-2"  no-gutters>
	    		<v-col>
	    			<v-btn tile text v-if="can_bulk_delete" outlined @click="$emit('delete-selected')">DELETE SELECTED <v-icon right small>delete</v-icon></v-btn>
	    		</v-col>
