<template>
    <div>
        <v-row>
            <v-col md="12">
                <vue-dropzone ref="dropzone" id="dropzone" :options="options" v-bind="$attrs"
                              @vdropzone-thumbnail="$emit('thumbnail', arguments)"
                              @vdropzone-file-added="file_is_uploaded(arguments)" @vdropzone-success="upload_success"
                              @vdropzone-error="file_error" @vdropzone-removed-file="file_is_removed()"
                              @vdropzone-canceled="file_is_removed()" @vdropzone-sending="file_is_sending"
        ...
        methods: {
            ...
            file_error(file, message, xhr) {
                ...
                if (message.errors && message.errors.file)
                    errorMessage = message.errors.file.join(',')
